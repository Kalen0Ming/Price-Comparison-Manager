...
import {
  ...
  ArrowUpDown, Equal, ArrowUp, ArrowDown, ChevronRight, AlertCircle,
} from "lucide-react";
...
function ProductCard({ label, data, side }: { label: string; data: Record<string, unknown>; side: "A" | "B" }) {
  ...
  const shownKeys = [nameKey, priceKey, sourceKey, urlKey, `product${side}_name`, `product${side}_price`, `product${side}_source`, `product${side}_url`];
  const otherFields = Object.entries(data).filter(([k]) => {
    return k.toLowerCase().includes(side.toLowerCase()) && !shownKeys.includes(k);
  });
    ...
    <Card className={`border-t-4 ${borderColor} h-full`}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base">
            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold mr-2 ${badgeColor}`}>
              商品 {label}
            </span>
          </CardTitle>
          {source !== "—" && (
      ...
      <CardContent className="space-y-3">
        ...
        <div>
          ...
          <p className="text-2xl font-bold text-foreground" data-testid={`text-product-${side}-price`}>
            {price !== "—" ? `¥${price}` : "—"}
          </p>
        </div>
...
export default function AnnotationWorkspace() {
  ...
  useEffect(() => {
    if (task?.existingAnnotation) {
      setResult(task.existingAnnotation.result as AnnotationResult);
  ...
  const isComplete = result.is_same_product !== undefined && result.price_comparison !== undefined;
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            标注选项
            {!isComplete && <span className="text-xs font-normal text-muted-foreground ml-1">（请填写全部必选项以提交）</span>}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-3">
              <OptionButton
                selected={result.is_same_product === "yes"}
                onClick={() => setResult(r => ({ ...r, is_same_product: "yes" }))}
                color="green"
                testId="option-same-yes"
              >
                ✓ 是同款
              </OptionButton>
              <OptionButton
                selected={result.is_same_product === "no"}
                onClick={() => setResult(r => ({ ...r, is_same_product: "no" }))}
                color="red"
                testId="option-same-no"
              >
                ✗ 非同款
              </OptionButton>
              <OptionButton
                selected={result.is_same_product === "uncertain"}
                onClick={() => setResult(r => ({ ...r, is_same_product: "uncertain" }))}
                color="amber"
                testId="option-same-uncertain"
              >
                ? 不确定
              </OptionButton>
            </div>
          ...
          <div>
            ...
            <div className="flex gap-3">
              <OptionButton
                selected={result.price_comparison === "A>B"}
                onClick={() => setResult(r => ({ ...r, price_comparison: "A>B" }))}
                color="red"
                testId="option-price-a-higher"
              >
              ...
              <OptionButton
                selected={result.price_comparison === "unknown"}
                onClick={() => setResult(r => ({ ...r, price_comparison: "unknown" }))}
                color="default"
                testId="option-price-unknown"
              >
                无法判断
              </OptionButton>
            </div>
          ...
          {/* Q3: Quality Comparison (optional) */}
          <div>
            ...
            <div className="flex gap-3">
              {(["A>B", "A<B", "A=B", "unknown"] as const).map((opt) => {
                ...
                return (
                  <OptionButton
                    key={opt}
                    selected={result.quality_comparison === opt}
                    onClick={() => setResult(r => ({ ...r, quality_comparison: opt }))}
                    color={colors[opt]}
                    testId={`option-quality-${opt.replace(">", "gt").replace("<", "lt").replace("=", "eq")}`}
                  >
                    {labels[opt]}
                  </OptionButton>
                );
          ...
          {Object.keys(result).length > 0 && (
            <div className="bg-muted/40 rounded-lg p-4 text-xs space-y-1">
              <p className="font-medium text-muted-foreground mb-2 uppercase tracking-wide">当前标注结果</p>
              {result.is_same_product && <p>是否同款：<span className="font-medium">{result.is_same_product === "yes" ? "是" : result.is_same_product === "no" ? "否" : "不确定"}</span></p>}
              {result.price_comparison && <p>价格对比：<span className="font-medium">{result.price_comparison}</span></p>}
              {result.quality_comparison && <p>质量对比：<span className="font-medium">{result.quality_comparison}</span></p>}
              {result.notes && <p>备注：<span className="font-medium">{result.notes}</span></p>}
            </div>
          )}
        </CardContent>
      </Card>
